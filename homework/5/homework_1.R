...
for (j in 1:num_variables){ # 1:20
  ...
  if (prev_error <=pred_error){
    print("--------------------------------------")
    print("-----no better than previous step-----")
    print("--------------------------------------")
    print("----------selected variables----------")
    print(selected_variable[-length(selected_variable)] +1)
      break
    }
  else{
    print("-----previous error vs current error-----")
    paste("previous: ",prev_error,"  current: ",pred_error)
